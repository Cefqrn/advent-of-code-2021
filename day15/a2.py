...
with open(os.path.join(os.path.dirname(__file__), "input")) as f:
    ...
    for dy in range(5):
        for row in data:
            data2.append([*map(lambda x: x + dy if x + dy < 10 else x + dy - 9, row), *map(lambda x: x + 1 + dy if x + 1 + dy < 10 else x + dy + 1 - 9, row), *map(lambda x: x + 2 + dy if x + 2 + dy < 10 else x + dy + 2 - 9, row), *map(lambda x: x + 3 + dy if x + 3 + dy < 10 else x + dy + 3 - 9, row), *map(lambda x: x + 4 + dy if x + 4 + dy < 10 else x + dy + 4 - 9, row)])
    data = data2
